...
function updateLoanRate(){ const type=document.getElementById('loan-type')?.value; const el=document.getElementById('loan-calculator-preview'); if(el && type){ const r=SPX_LOAN_RATES[type]||12.5; const amt=Number(document.getElementById('loan-amount')?.value||0); const tenure=Number(document.getElementById('loan-tenure')?.value||60); if(amt>=10000){ calculateLoanPreview(); } else { el.innerHTML=`<span>Interest rate</span><strong>${r.toFixed(2)}% p.a.</strong><small>Enter loan amount to estimate EMI.</small>`; } } }
async function calculateLoanPreview(force=false){ const amount=Number(document.getElementById('loan-amount')?.value||0), tenure=Number(document.getElementById('loan-tenure')?.value||60), type=document.getElementById('loan-type')?.value||'PERSONAL', box=document.getElementById('loan-calculator-preview'); if(!box)return; if(amount<10000){updateLoanRate();return;} try{const r=await fetch('/api/loans/calculate',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({loan_type:type,amount,tenure_months:tenure})});const d=await r.json();if(!d.success){box.innerHTML=`<span>Estimate</span><strong>${d.message}</strong>`;return;}box.innerHTML=`<span>Estimated EMI</span><strong>${moneyINR(d.emi)}/month</strong><small>${d.interest_rate.toFixed(2)}% p.a. · Total interest ${moneyINR(d.total_interest)} · Total payable ${moneyINR(d.total_payable)}</small>`;}catch(e){if(force)showToast('Unable to calculate EMI right now.','error');}}
let pendingLoanApplication=null;
function getLoanFormData(){return {loan_type:document.getElementById('loan-type')?.value,amount:Number(document.getElementById('loan-amount')?.value||0),tenure_months:Number(document.getElementById('loan-tenure')?.value||0),employment_type:document.getElementById('loan-employment')?.value,monthly_income:Number(document.getElementById('loan-income')?.value||0),existing_emi:Number(document.getElementById('loan-existing-emi')?.value||0),purpose:document.getElementById('loan-purpose')?.value.trim()||''};}
